...
def _parse_count(v) -> int:
    """'< 10' 같은 비정상 문자열 포함한 네이버 API 카운트 값을 int로 변환"""
    if v is None:
        return 0
    try:
        return int(v)
                    ...
                    if keyword_list:
                        ...
                        for item in keyword_list:
                            ...
                            if rel == keyword.lower() or rel == hint_keyword.lower():
                                return {
                                    "keyword": keyword,
                                    "monthly_search_pc": _parse_count(item.get("monthlyPcQcCnt")),
                                    "monthly_search_mobile": _parse_count(item.get("monthlyMobileQcCnt")),
                        ...
                        return {
                            "keyword": keyword,
                            "monthly_search_pc": _parse_count(first_item.get("monthlyPcQcCnt")),
                            "monthly_search_mobile": _parse_count(first_item.get("monthlyMobileQcCnt")),
